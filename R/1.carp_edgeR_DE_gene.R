...
d <- data[keep,] 
colnames(d)
conditions <- factor(gsub(".1","",colnames(d))) ## 将“.1” 替换掉

### 若存在重复样品，需要更改每一列的样品名称，参考以下 ##################
...
for(i in 1:length(t)){  
    ###  get paires ，  这一步应根据需要更改！！
    test <- t[[i]] ##get names of test pairs
    comp <- paste(test[2],test[1],sep = "-")   ## 注意!!比对结果为 test[2]-test[1] 
    comp
