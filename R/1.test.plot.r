...
for (i in 1:length(site)) {
    ...
    s1$fish_id <- substr(s1$id,1,2)
    ns1 <- ncol(s1)
    ss <- merge(ss,s1[,c(ns1,ns1-1)],all = T)

}
ss  # SNP 的genotype 和 phenotype文件，类似ped格式文件
map # SNP 位置信息文件，类map格式文件
write.table(ss,file = paste(file.name,".ped.het1",sep=""),quote = F,sep="\t",row.names = F)
write.table(map,file = paste(file.name,".map",sep=""),quote = F,sep="\t",row.names = F)
