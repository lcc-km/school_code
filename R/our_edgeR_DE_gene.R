...
d <- data[keep,]
colnames(d)
conditions <- factor(gsub(".1","",colnames(d))) ## =+!0.1!1 Lf;;5t
...
## IhVCPhR*1H=O2nRlgene5DWi1p (JJSCSZC;SPVX845DQy1>)

## opt 1: partly compare
t <- list()
for(i in 1:length(conditions)){
  t[[i]] <- c(as.character(conditions[i]),as.character(conditions[29]))
  }

## opt 2: all compare
cond <- levels(conditions)
t <- list()
time=0
...
for(i in 1:length(t)){
    ...
    et0 <- exactTest(exp_study1,pair = test)
    topTags(et0)
